...
export default function AudioStreamPlayer({
  ...
  const connect = async () => {
    try {
      ...
      ws.onopen = () => {
        console.log("[AudioStreamPlayer] WebSocket connected");
        setIsConnected(true);
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.error("[AudioStreamPlayer] Error parsing message:", err);
        }
      };

      ws.onerror = (err) => {
        console.error("[AudioStreamPlayer] WebSocket error:", err);
      ...
      ws.onclose = () => {
        console.log("[AudioStreamPlayer] WebSocket closed");
  ...
  const handleWebSocketMessage = async (message: any) => {
    ...
    try {
      ...
      if (message.event === "connected") {
        console.log("[AudioStreamPlayer] Stream connected, version:", message.version);
        return;
      }

      if (message.event === "start") {
        console.log("[AudioStreamPlayer] Stream started:", message.start);
        setIsPlaying(true);
        return;
      }
      ...
      if (message.event === "stop") {
        console.log("[AudioStreamPlayer] Stream stopped");
        setIsPlaying(false);
        return;
      }
      ...
      console.error("[AudioStreamPlayer] Error processing message:", err);
    }
  };

  const processAudioChunk = async (media: any) => {
    if (!audioContextRef.current || !media.payload) return;
